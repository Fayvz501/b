...
def _find_font(filename: str) -> str:
    """Ищем .ttf в ./fonts/, иначе берём из matplotlib."""
    local = os.path.join(_PROJECT_ROOT, "fonts", filename)
    ...
    raise FileNotFoundError(
        f"Не найден шрифт {filename}. Положи в ./fonts/ "
        f"или убедись, что matplotlib установлен."
    )


# регистрируем один раз при импорте
...
def build_report(amount: float, rate: float, months: int,
    ...
    summary = [
        ...
        ["Первый платёж", f"{ann.first_payment:,.2f} ₽",
         f"{diff.first_payment:,.2f} ₽"],
        ["Последний платёж", f"{ann.last_payment:,.2f} ₽",
         f"{diff.last_payment:,.2f} ₽"],
        ["Всего выплачено", f"{ann.total_paid:,.2f} ₽",
         f"{diff.total_paid:,.2f} ₽"],
        ["Переплата", f"{ann.total_interest:,.2f} ₽",
         f"{diff.total_interest:,.2f} ₽"],
